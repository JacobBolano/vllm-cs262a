...
launch_disagg_prefill() {
  ...
  CUDA_VISIBLE_DEVICES=1 python3 \
    ...
    --gpu-memory-utilization 0.6 \
    --kv-transfer-config \
    '{"kv_connector":"PyNcclConnector","kv_role":"kv_consumer","kv_rank":1,"kv_parallel_size":4,"kv_buffer_size":4e9}' &
  ...
  CUDA_VISIBLE_DEVICES=3 python3 \
    ...
    --gpu-memory-utilization 0.6 \
    --kv-transfer-config \
    '{"kv_connector":"PyNcclConnector","kv_role":"kv_consumer","kv_rank":2,"kv_parallel_size":4,"kv_buffer_size":4e9}' &
  ...
  python3 disagg_1p3d_proxy_server.py &
  sleep 1
}
...
main() {
  ...
  # rm -rf results_1p3d_sharegpt
  # mkdir results_1p3d_sharegpt
  ...
  for qps in 15; do
      ...
      for num_reqs in 50 500; do
        ...
        if [ $counter -gt $start_point ]; then
          ...
          benchmark $qps disagg_prefill_dist_gloo "./results_1p3d_sharegpt" $num_reqs
          kill_gpu_processes
          sleep 5
        else
