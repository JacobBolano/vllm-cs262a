...
launch_disagg_prefill() {
  ...
  #   --gpu-memory-utilization 0.6 \
  #   --kv-transfer-config \
  #   '{"kv_connector":"MooncakeConnector","kv_role":"kv_producer","kv_rank":0,"kv_parallel_size":2,"kv_buffer_size":5e9}' &
  ...
  #   --gpu-memory-utilization 0.6 \
  #   --kv-transfer-config \
  #   '{"kv_connector":"MooncakeConnector","kv_role":"kv_consumer","kv_rank":1,"kv_parallel_size":2,"kv_buffer_size":5e9}' &
  # # decode #2
...
benchmark() {
  ...
  python3 ../benchmark_serving.py \
          ...
          --result-filename "$tag"-qps-"$qps"-prompts-"$num_prompts"-input-"$input_len"-output-"$output_len".json \
          --ignore-eos \
          --request-rate "$qps"
...
main() {
  ...
  rm -rf results_disagg_3p1d
  mkdir results_disagg_3p1d
  ...
  for qps in 14; do
    for num_prompts in 200; do
      for pair in "${input_output[@]}"; do
        kill_gpu_processes
        launch_disagg_prefill
        read input_len output_len <<< "$pair"
        benchmark $qps $output_len disagg_prefill "./results_disagg_3p1d" $num_prompts $input_len
      done
    done
  done
  kill_gpu_processes
