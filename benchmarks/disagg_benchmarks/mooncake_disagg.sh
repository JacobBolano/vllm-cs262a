  ...
  for qps in "$@"; do
    for pair in "${pairs[@]}"; do
        ...
        python3 ../benchmark_serving.py \
              ...
              --result-filename=${file_prefix}-input-${input_len_name}-output-${output_len_name}-qps-${qps}-serving.json \
              2>&1 | tee ${logs_root}/${file_prefix}-${input_len_name}-output-${output_len_name}-[qps]-${qps}-serving.txt
        sleep 2
      done
    done
}
...
main() {
  ...
  benchmark proxy-1-1 2 8 14

  launch_disagg_proxy 3 1
  benchmark proxy-3-1 2 8 14
