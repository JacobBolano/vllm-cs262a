...
class PyNcclPipe(KVPipeBase):
    ...
    def __init__(self,
        ...
        self.target_rank_for_send = -1
        self.target_rank_for_recv = -1
    ...
    def _send_metadata(self, metadata: Metadata, target_rank):
        """
    ...
    def _recv_metadata(self, source_rank:int) -> Metadata:
        """
        ...
        Returns:
            - metadata: A dictionary with keys "dtype" and "shape" describing
              the tensor.
        """
        return self.group.recv_obj(source_rank)
    ...
    def _recv_impl(self, source_rank: int) -> Optional[torch.Tensor]:
        """
        ...
        """
        metadata = self._recv_metadata(source_rank)
        if metadata["dtype"] is None:
            return None
        buffer = self._prepare_recv_buffer(metadata)
        self.device_recv_func(buffer, source_rank)

        return buffer

    def send_tensor_wrapper(self, tensor: Optional[torch.Tensor],
                            tensor_size: int, target_rank) -> None:
        """
    ...
    def block_if_full(self):
        ...
        while self.buffer_size > self.buffer_size_thresh:
            logger.info("KV cache transfer pipe is full. Waiting...")
            time.sleep(0.05)

    def send_tensor(self, tensor: Optional[torch.Tensor], target_rank: Optional[int] = None) -> None:
        """
        ...
        target = target_rank if target_rank is not None else self.target_rank_for_send

        # if target_rank is not None:
        #     self.target_rank_for_send = target_rank
        ...
        # Pass 'target' to _send_impl
        self.transport_thread.submit(self.send_tensor_wrapper, tensor, tensor_size, target)

    def recv_tensor(self, source_rank = None) -> Optional[torch.Tensor]:
        """
        ...
        # This is triggered when a kv cache transfer has failed
        source = source_rank if source_rank is not None else self.target_rank_for_recv

        future = self.transport_thread.submit(self._recv_impl, source)
        ...
        logger.info(f"Jacob recv tensor from {source_rank} at time {timestamp} and {utc_time}")
