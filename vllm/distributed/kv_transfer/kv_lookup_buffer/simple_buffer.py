...
#                 logger.info("ROHAN Prefill Begins Drop Select Handler: %f (Unix), %s (UTC)", timestamp, utc_time)
...
#                             self._send_tensor_and_dec_size(tensor, recv_rank)
#                         self.buffer_cv.notify()
#                     else:
#                         self._send_tensor_and_dec_size(input_tokens, recv_rank)
#                         self._send_tensor_and_dec_size(roi, recv_rank)
#                         # Send empty tensors as flags
#                         for _ in range(3):  # key, value, hidden
#                             self._send_tensor_and_dec_size(torch.tensor([]), recv_rank)
#                         logger.info("Shri Signaled KV cache miss to decode instance")
...
#             self.data_pipe.send_tensor(input_tokens, target_ranks[t_idx])
#             self.data_pipe.send_tensor(roi, target_ranks[t_idx])
...
#             if key.numel() > 0:  # Empty tensor check
#                 found = True
#             else:
#                 t_idx += 1
#                 if t_idx >= len(target_ranks):
#                     t_idx = 0
#                     logger.info("Shri could not find KV cache in all prefill instances, relooping")
